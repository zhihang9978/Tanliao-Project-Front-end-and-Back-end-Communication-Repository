...
更新时间：2026-05-04 14:00 +08:00 (06:00 UTC)
执行方：Codex
范围：使用本地 `D:\tantan` 构建产物；服务器端处理后端已确认的三套前端静态目录，并额外清理同一 Web runtime 下的 legacy `myres/common.js` 旧参数残留。
...
| `D:\tantan\bs-page\page\myres\common.js` | `/opt/tantan/runtime/web/myres/common.js` | ✅ 已单文件上传并校验 |
...
另外，Codex 复查到 legacy `myres/common.js` 也保留了 `bs_tio_session` 兼容逻辑。该文件不属于三套 dist，但位于同一 Web runtime 静态根下，安全上不应继续保留旧 session URL 参数，已一并移除并上传。
...
| `/opt/tantan/runtime/web/myres/` | 1 | 30727 | `common.js` | `69e0fe1b4e654ae0f0a959855650a7dd` |
...
- `myres/common.js` 单文件上传前保留服务器备份，并在上传后比对 MD5。
...
```bash
grep -RIlE 'ndapi/autologin|ndapiAutologin|bs_tio_session' \
  /opt/tantan/runtime/web/myres/common.js
```
...
- `D:\tantan\bs-page\page\myres\common.js`
